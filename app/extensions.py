...
class Extensions():
    ...
    def append_app(self, user_id, app_id, data):
        ...
        for method in self.apps[app_id]['executable_methods']:
            ...
            if (method_split[0] == 'io'):
                self.socketio.on(method_split[1], lambda data=None, method=getattr(executable, method), user_id=int(user_id): self.socket_wrapper(data, method, user_id), namespace=app_namespace)
                pass
    ...
    def set_config_for_app(self, app_id, config):
        ...
        config_class = Config(path + "config.ini", {"Extension": self.apps_admin[app_id]["config"]})
        ...
        for dir in folders:
            try:
                ...
                if (settings_json["type"] == 'application'):
                    ...
                    config = Config(self.path + dir + "/config.ini", config).options['Extension']
                    if (config['use']):
...
document.head.innerHTML += '<link rel="icon" href="/static/img/favicon/app.svg" type="image/svg+xml">';\n
    ...
    # --------------------------------------------------------------------------
    def socket_wrapper(self, data, method, user_id, username):
        try:
            method(data)
        except Exception as e:
